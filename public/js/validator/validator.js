...
(function(root, factory) {
    ...
    FormValidator.prototype = {
        ...
        texts: {
            ...
            short: 'el campo es muy corto',
            long: 'el campo es muy largo',
            checked: 'debe ser marcado',
            empty: 'el campo no puede estar vacío',
            select: 'Por favor, escoja una opción',
            number_min: 'muy corto',
            number_max: 'muy largo',
            url: 'invalida URL',
            number: 'no es un número',
            email: 'el email es invélido ',
            email_repeat: 'los emails no coinciden',
            date: 'fecha inválida',
            time: 'tiempo inválido',
            password_repeat: 'las contraseñas no coinciden',
            no_match: 'los campos no coinciden',
            complete: 'entrada no cumpleta'
        },
        ...
        tests: {
            ...
            acronimo: function(field, data) {
                let id = $(field).attr('parent');
                let sized = $("#" + id).val();
                if (sized.length < data.value.length && data.value.length > 7) {
                    return "demasiado extenso"
                }
        ...
        checkField: function(field, silent) {
            ...
            if (data.valid && field.type == "password" && $(field).attr('pass') == 'true') {
                let resta = validarPass(field)
                data.valid = resta.status;
                testResult = resta.msg;
            }
