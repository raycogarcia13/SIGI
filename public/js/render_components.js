function renderComponents(){
    ...
    const iconCheckOn = base_path+"images/iconos/config/S_Btn_selectRow-ActiveDark_On.svg"
    ...
    $('.edit-select').each(function(){
        ...
        let object = $(this)
        let currentValue = (object.attr('current-value')) ? `<a href="#" class="current-selected" value="` + object.attr('current-value').split(':')[1]+`"><img src="`+desplegar+`" width="15" style="margin-right: 7px;">` + object.attr('current-value').split(':')[0]+`</a>` :`<a href="#" class="current-selected" value=""><img src="`+desplegar+`" width="15" style="margin-right: 7px;">Selecciona:</a>`
        ...
        object.off().on('click',function(){
            // Ocultar todos los input select
            $('.input-select').hide()
        ...
        object.children('.input-select').children('a').off().on('click',function(){
            ...
            currentSelected.attr('value',thisObject.data('value'))
            inputSelect.hide()
            return false
        })
